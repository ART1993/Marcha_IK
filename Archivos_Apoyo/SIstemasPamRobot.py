...
def Sistema_Musculos_PAM_16(control_joint_names=None, max_pressure=5):
    ...
    for joint in control_joint_names:
        if "hip_roll" in joint:
            pam_dict[f"{joint}_flexor"] = PAMMcKibben(L0=0.6, r0=0.035, alpha0=np.pi/4,max_pressure=max_pressure)
            pam_dict[f"{joint}_extensor"] = PAMMcKibben(L0=0.6, r0=0.032, alpha0=np.pi/4,max_pressure=max_pressure)
        elif "hip_pitch" in joint:
            pam_dict[f"{joint}_flexor"] = PAMMcKibben(L0=0.6, r0=0.030, alpha0=np.pi/4,max_pressure=max_pressure)
            pam_dict[f"{joint}_extensor"] = PAMMcKibben(L0=0.6, r0=0.035, alpha0=np.pi/4,max_pressure=max_pressure)
        elif "knee" in joint:
            pam_dict[f"{joint}_flexor"] = PAMMcKibben(L0=0.5, r0=0.036, alpha0=np.pi/4,max_pressure=max_pressure)
            pam_dict[f"{joint}_extensor"] = PAMMcKibben(L0=0.5, r0=0.036, alpha0=np.pi/4,max_pressure=max_pressure)
        elif "ankle_roll" in joint:
            pam_dict[f"{joint}_flexor"] = PAMMcKibben(L0=0.3, r0=0.035, alpha0=np.pi/4,max_pressure=max_pressure)
            pam_dict[f"{joint}_extensor"] = PAMMcKibben(L0=0.3, r0=0.035, alpha0=np.pi/4,max_pressure=max_pressure)
        elif "ankle_pitch" in joint:
            pam_dict[f"{joint}_flexor"] = PAMMcKibben(L0=0.3, r0=0.045, alpha0=np.pi/4,max_pressure=max_pressure)
            pam_dict[f"{joint}_extensor"] = PAMMcKibben(L0=0.3, r0=0.045, alpha0=np.pi/4,max_pressure=max_pressure)
    return pam_dict
