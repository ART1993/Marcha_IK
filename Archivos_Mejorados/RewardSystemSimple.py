...
class UltraSimpleActionSelector:
    ...
    def __init__(self, env):
        ...
        # NUEVOS PATRONES PARA EQUILIBRIO EN UNA PIERNA
        self.action_patterns = {
            SingleLegActionType.BALANCE_LEFT_SUPPORT: {
                # Pierna izquierda de soporte, derecha levantada
                'pam_pressures': [0.6, 0.4, 0.3, 0.7, 0.1, 0.6],  # Flexor derecho para levantar pierna
                'description': 'Equilibrio en pierna izquierda'
            },
            SingleLegActionType.BALANCE_RIGHT_SUPPORT: {
                # Pierna derecha de soporte, izquierda levantada  
                'pam_pressures': [0.3, 0.7, 0.6, 0.4, 0.6, 0.1],  # Flexor izquierdo para levantar pierna
                'description': 'Equilibrio en pierna derecha'
            },
            SingleLegActionType.TRANSITION: {
                # Posición intermedia para cambio de pierna
                'pam_pressures': [0.4, 0.4, 0.4, 0.4, 0.3, 0.3],
                'description': 'Transición entre piernas'
            }
        }
        
        # ===== ESTADO INTERNO MÍNIMO =====
        self.current_action = SingleLegActionType.BALANCE_LEFT_SUPPORT
        self.transition_counter = 0  # Para alternar piernas
    ...
    def decide_current_action(self):
        ...
        if len(self.last_10_rewards) < 5:
            self.current_action = SimpleActionType.BALANCE_LEFT_SUPPORT
            return
        
        # Lógica simple: si está yendo bien con balance, probar sentadilla ocasionalmente
        recent_avg = np.mean(self.last_10_rewards[-5:])
        
        # Si va bien, ocasionalmente cambiar de pierna
        if recent_avg > 3.0:  # Performance buena
            self.transition_counter += 1
            if self.transition_counter > 50:  # Cada ~50 steps, cambiar pierna
...
class SingleLegBalanceRewardSystem:
    ...
    Reemplaza UltraSimpleRewardSystem para esta tarea específica.
    """
...
class SingleLegActionSelector:
    ...
    Reemplaza UltraSimpleActionSelector para esta tarea.
    """
